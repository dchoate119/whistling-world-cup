...
def main():
    ...
    spec = np.full((len(detector.band_freqs), n_frames), -100.0)  # dB, newest column on the right
    ...
    def update(_):
        ...
        ax.set_title("No whistle" if peak_freq is None else f"Whistle {peak_freq:.0f} Hz")

    anim = FuncAnimation(fig, update, interval=20, cache_frame_data=False)  # must stay referenced or it stops
    try:
        plt.show()
